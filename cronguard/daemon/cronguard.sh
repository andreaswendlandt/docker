...
error_logfile="/var/log/cronguard_error.log"
pid="$$"
interval="60"
...
# Cronguard Function
cronguard() {
    query=$(mysql -h $db_host -u $db_user -p$db_password $db -e"SELECT token,result,host FROM $table" 2>&1 | grep -v 'Using a password' | tail -n +2)
    if ! [ -z "$query" ]; then
        echo -e "$query" | while read line; do
            set $line
	    if [ "$2" == "success" ]; then
                log "$1 will be removed"
	        if mysql -h $db_host -u $db_user -p$db_password $db -e"DELETE FROM $table WHERE token = \"$1\"" 2>&1 | grep -v 'Using a password'; then
                    log "$1 from $3 removed"
                fi
            elif [ "$2" == "fail" ]; then
                log "$1 will be logged"
                failed_command=$(mysql -h $db_host -u $db_user -p$db_password $db -e"SELECT command FROM $table WHERE token =\"$1\"" 2>&1 | grep -v 'Using a password' | tail -n +2)
                if error_log "ERROR, failed Cronjob on $3: \"$failed_command\""; then
	            mysql -h $db_host -u $db_user -p$db_password $db -e"DELETE FROM $table WHERE token = \"$1\"" 2>&1 | grep -v 'Using a password'
	        fi    
            elif [ "$2" == "NULL" ]; then
                log "$1 is still running"
	        current_time=$(date +%s)
                start_time=$(mysql -h $db_host -u $db_user -p$db_password $db -e"SELECT start_time FROM $table WHERE token =\"$1\"" 2>&1 | grep -v 'Using a password' | tail -n +2)
	        running_since=$((current_time-start_time))
                if [ $running_since -ge 86400 ]; then
                long_running_command=$(mysql -h $db_host -u $db_user -p$db_password $db -e"SELECT command FROM $table WHERE token =\"$1\"" 2>&1 | grep -v 'Using a password' | tail -n +2)
                    if error_log "ERROR, long running Cronjob on $3 (longer than one day $running_since seconds): \"$long_running_command\""; then
                        mysql -h $db_host -u $db_user -p$db_password $db -e"DELETE FROM $table WHERE token = \"$1\"" 2>&1 | grep -v 'Using a password'
	            fi
                fi
            fi
